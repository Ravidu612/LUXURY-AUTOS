...
const VehicleBooking = () => {
  ...
  useEffect(() => {
    const fetchVehicle = async () => {
      try {
        const response = await axios.get(`http://localhost:4001/Vehicles/${VehicleId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(URL, {
        TicketId: ticketId,
        count: Number(count),
        VehicleId: Vehicle?.name,  // Send the Vehicle name instead of the ID
        userId,
        showTimeId,
        date,
        seat,
  ...
  const isVehicleAvailable = Vehicle && Vehicle.status === 'available'; // Check if the Vehicle is available
        ...
        <Grid container spacing={4}>
            ...
            {Vehicle && (
              <Card sx={{ boxShadow: '0 4px 10px rgba(0,0,0,0.1)' }}>
                <CardMedia
                  ...
                  alt={Vehicle.name}
                  height="350"
                  image={Vehicle.image || 'http://localhost:5173/src/Components/Images/3.png'}
                  title={Vehicle.name}
                  sx={{ objectFit: 'cover' }}
                />
                <CardContent>
                  <Typography variant="h5" sx={{ fontWeight: 'bold', marginBottom: 1 }}>{Vehicle.name}</Typography>
                </CardContent>
              </Card>
            )}
          </Grid>
          ...
          <Grid item xs={12} md={8}>
            ...
            <Typography variant="body1" gutterBottom sx={{ marginBottom: 2 }}>
              Fill in your details below to book tickets for <strong>{Vehicle ? Vehicle.name : 'the Vehicle'}</strong>.
            </Typography>

            {!isVehicleAvailable ? ( // Check if the Vehicle is available
              <Typography variant="body1" color="error">
                Sorry, this Vehicle is not available for booking.
              </Typography>
            ) : (
